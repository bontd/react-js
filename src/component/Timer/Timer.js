...
class Timer extends React.Component {
    ...
    handleDisplayFocusChange(uniOfTime) {
        this.setState(() => ({uniOfTime}))
    }

    handleKeypadClick(time) {
        if(this.state.uniOfTime === 'HH'){
            this.setHours(time);
        }else if(this.state.uniOfTime === 'MM'){
            this.setMinutes(time);
        }else if(this.state.uniOfTime === 'SS'){
    ...
    canStart(){
        this.setState((prevState) => ({
            canStart: prevState.state !== 'STARTED' && (parseInt(prevState.hours) > 0
